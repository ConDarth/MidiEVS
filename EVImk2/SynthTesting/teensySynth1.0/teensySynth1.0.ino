#include <Audio.h>
#include <Wire.h>
#include <SPI.h>
#include <SD.h>
#include <SerialFlash.h>

// GUItool: begin automatically generated code
AudioSynthWaveformModulated Osc1Voice1;   //xy=178.33333333333331,394.99999999999994
AudioSynthNoiseWhite     NoiseVoice1;         //xy=179,478
AudioSynthWaveformModulated Osc2Voice1; //xy=183,436
AudioMixer4              MixerVoice1;         //xy=350,443
AudioOutputI2S           i2s1;           //xy=524.0000305175781,439.8888244628906
AudioConnection          patchCord1(Osc1Voice1, 0, MixerVoice1, 0);
AudioConnection          patchCord2(NoiseVoice1, 0, MixerVoice1, 2);
AudioConnection          patchCord3(Osc2Voice1, 0, MixerVoice1, 1);
AudioConnection          patchCord4(MixerVoice1, 0, i2s1, 0);
AudioConnection          patchCord5(MixerVoice1, 0, i2s1, 1);
AudioControlSGTL5000     sgtl5000_1;     //xy=368.88888888888886,341.6666666666667
// GUItool: end automatically generated code

#define BUFFER 16


const float noteFreqs[128] = {8.176, 8.662, 9.177, 9.723, 10.301, 10.913, 11.562, 12.25, 12.978, 13.75, 14.568, 15.434, 16.352, 17.324, 18.354, 19.445, 20.602, 21.827, 23.125, 24.5, 25.957, 27.5, 29.135, 30.868, 32.703, 34.648, 36.708, 38.891, 41.203, 43.654, 46.249, 48.999, 51.913, 55, 58.27, 61.735, 65.406, 69.296, 73.416, 77.782, 82.407, 87.307, 92.499, 97.999, 103.826, 110, 116.541, 123.471, 130.813, 138.591, 146.832, 155.563, 164.814, 174.614, 184.997, 195.998, 207.652, 220, 233.082, 246.942, 261.626, 277.183, 293.665, 311.127, 329.628, 349.228, 369.994, 391.995, 415.305, 440, 466.164, 493.883, 523.251, 554.365, 587.33, 622.254, 659.255, 698.456, 739.989, 783.991, 830.609, 880, 932.328, 987.767, 1046.502, 1108.731, 1174.659, 1244.508, 1318.51, 1396.913, 1479.978, 1567.982, 1661.219, 1760, 1864.655, 1975.533, 2093.005, 2217.461, 2349.318, 2489.016, 2637.02, 2793.826, 2959.955, 3135.963, 3322.438, 3520, 3729.31, 3951.066, 4186.009, 4434.922, 4698.636, 4978.032, 5274.041, 5587.652, 5919.911, 6271.927, 6644.875, 7040, 7458.62, 7902.133, 8372.018, 8869.844, 9397.273, 9956.063, 10548.08, 11175.3, 11839.82, 12543.85};
uint8_t noteState[1] = {0};
uint8_t velocity[1] = {100} ;
float noteAmplitude[1] = {0} ;

float DIV127 = 1.0 / 127.0 ;

void setup() {
  // put your setup code here, to run once:
  AudioMemory(50) ;
  usbMIDI.setHandleControlChange(myControlChange);
  usbMIDI.setHandleNoteOff(myNoteOff);
  usbMIDI.setHandleNoteOn(myNoteOn);
  sgtl5000_1.enable() ;
  sgtl5000_1.volume(0.3) ;
  
  Osc1Voice1.begin(WAVEFORM_TRIANGLE) ; 
  Osc2Voice1.begin(WAVEFORM_SINE) ; 
  NoiseVoice1.amplitude(1.0) ;

  MixerVoice1.gain(0, 1.0);
  MixerVoice1.gain(1, 1.0);
  MixerVoice1.gain(2, 0.0);
}

void loop() {
  usbMIDI.read();  
}

void myNoteOff(byte channel, byte note, byte velocity) {
  keyBuff(note, false) ;
}

void myNoteOn(byte channel, byte note, byte velocity) {
  keyBuff(note, true) ;
}

void myControlChange(byte channel, byte control, byte velocity) {
  
}

void keyBuff(byte note, bool state) {
  static byte buff[BUFFER];
  static byte buffSize = 0;

  // Add Note
  if (state == true && (buffSize < BUFFER) ) {
    oscPlay(note);
    buff[buffSize] = note;
    buffSize++;
    return;
  }

  // Remove Note
  else if (state == false && buffSize != 0) {
    for (byte found = 0; found < buffSize; found++) {
      if (buff[found] == note) {
        for (byte gap = found; gap < (buffSize - 1); gap++) {
          buff[gap] = buff[gap + 1];
        }
        buffSize--;
        buff[buffSize] = 255;
        if (buffSize != 0) {
          oscPlay(buff[buffSize - 1]);
          return;
        }
        else {
          oscStop();
          return;
        }
      }
    }
  }

  return ;
}

void oscPlay(byte note) {
  Osc1Voice1.frequency(noteFreqs[note]) ;
  Osc2Voice1.frequency(noteFreqs[note]) ;
  noteAmplitude[0] = (float) velocity[0] * DIV127 ;
  Osc1Voice1.amplitude(noteAmplitude[0]) ;
  Osc2Voice1.amplitude(noteAmplitude[0]) ;
  NoiseVoice1.amplitude(noteAmplitude[0]) ;
}

void oscStop() {
  Osc1Voice1.amplitude(0.0);
  Osc2Voice1.amplitude(0.0);
  NoiseVoice1.amplitude(0.0) ;
}
